...
public class NetworkGenerator implements INetworkGenerator {
    ...
    private void gradualIncreaseOfNeuronsUpperBound(int i) {
        ...
        if (i < layer.length * 0.6) {
            numOfFilters = 128;
        }
        if (i < layer.length * 0.8) {
            numOfFilters = 256;
        }
        if (i >= layer.length * 0.8) {
            numOfFilters = 512;
    ...
    private String appendLastLayers(StringBuilder str, TreeSet<AbstractLayer> set) {
        if (!outputShape.equals("siamese")) {
            if (flattened) {
                str.append("layer_last = Dense(" + outputShape + ", activation='softmax')(" + set.last().getLayerId()
                        + ")\n");
                ...
                str.append("layer_last = Dense(" + outputShape + ", activation='softmax')(flatten)\n");
    ...
    private void appendImports(StringBuilder str) {
        str.append("# Generated by automatic network generator --- created by T.Hipca, 2019, as a part of masters thesis\n");
        str.append("from tensorflow.python.keras.models import Model\n");
        str.append("from tensorflow.python.keras.layers import *\n");
        str.append("\n");
    }
